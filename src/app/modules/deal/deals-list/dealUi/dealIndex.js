...
const DealIndex = () => {
  ...
  const deals = dealsData.filter((val) => {
    ...
    if (
      val?.campaignSource?.campaignName
        ?.toLowerCase()
        ?.includes(searchTerm?.toLowerCase())
    ) {
      console.log(val, "val");
    ...
    <>
      <div className="tab-content">
        <Board
          ...
          renderCard={(
            { id, dealName, dealContactPersonName },
            { dragging }
          ) => (
            <div
              style={{
