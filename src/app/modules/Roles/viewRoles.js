...
import { fetchRoleById, fetchUsersByCompanyId } from "../user-management/users-list/_redux/userAction";
import {
  fetchAllModules,
	...
	return(<tr>
	   ...
	   <td class="d-flex align-items-center">
		   ...
		   <div class="symbol symbol-circle symbol-50px overflow-hidden me-3">
			   <a href="../../demo6/dist/apps/user-management/users/view.html">
				   <div class="symbol-label">
					   <img src={`http://65.2.10.157:5377${img}`} alt={userName} class="w-100" />
				   </div>
			   </a>
		   </div>
			{/*end::Avatar */}
		   ...
		   <div class="d-flex flex-column">
			   <a href="../../demo6/dist/apps/user-management/users/view.html" class="text-gray-800 text-hover-primary mb-1">{userName}</a>
			   <span>{email}</span>
		   </div>
			{/*begin::User details */}
	   </td>
...
function ViewRoles() {
  ...
  const token = useSelector((state) => state?.auth?.authToken);const userAdmin = useSelector(
  ...
  const UsersByRole=UsersByCompanyId.filter((item)=>item?.rolebasedaccess?.role===RoleById?.role)
  const companyId=userAdmin?.company?.id
  console.log(RoleById,"ASD");
  
  useEffect(()=>{dispatch(fetchUsersByCompanyId(companyId,token))},[])
									...
									<div class="card card-flush">
										...
										<div class="card-header">
											...
											<div class="card-title">
												<h2 class="mb-0">{RoleById?.role}</h2>
											</div>
											 {/*end::Card title */}
										</div>
										 {/*end::Card header */}
									...
									<div class="card card-flush mb-6 mb-xl-9">
										...
										<div class="card-header pt-5">
											...
											<div class="card-title">
												...
												<span class="text-gray-600 fs-6 ms-1">({UsersByRole?.length})</span></h2>
											</div>
											 {/*end::Card title */}
											 {/*begin::Card toolbar */}
										...
										<div class="card-body pt-0">
											...
											<table class="table align-middle table-row-dashed fs-6 gy-5 mb-0" id="kt_roles_view_table">
												...
												<tbody class="fw-semibold text-gray-600">
													{UsersByRole.map((item)=> <UserDetailsComponent img={item?.image?.url} userName={item?.username} email={item?.email} />)}
												</tbody>
											</table>
										</div>
									</div>
