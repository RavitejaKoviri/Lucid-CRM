...
function PostModal(props: any) {
  ...
  const getUserId = (accessId: any) => {
    axios
      .get(`https://graph.facebook.com/v6.0/me?access_token=${accessId}`)
      .then((response) => {
        const { data } = response;
  ...
  const getPageAcessToken = (id: any, accessId: any) => {
    axios
      .get(
        `https://graph.facebook.com/v6.0/${id}/accounts?access_token=${accessId}`
      )
      .then((response) => {
        const { data } = response;
  ...
  const PostToFacebook = () => {
    axios
      .post(
        `https://graph.facebook.com/101750425879367/photos?message=${data}&url=http://65.2.10.157:5377${imageUrl}&access_token=${accessToken}`
      )
      .then((response) => {
        const { data } = response;
        console.log(data, "data");
  ...
  const getInstaId = (token: any) => {
    axios
      .get(
        `https://graph.facebook.com/v15.0/101750425879367/?fields=instagram_business_account&access_token=${token}`
      )
      .then((response) => {
        const { data } = response;
  ...
  const createMediaContainer = (id: any, token: any) => {
    axios
      .post(
        `https://graph.facebook.com/v15.0/${id}/media?image_url=http://65.2.10.157:5377${imageUrl}&caption=%23${data}&access_token=${token}`
      )
      .then((response) => {
        const { data } = response;
  ...
  const postInstagramPost = (mediaId: any, id: any, token: any) => {
    axios
      .post(
        `https://graph.facebook.com/v15.0/${id}/media_publish?access_token=${token}&creation_id=${mediaId}`
      )
      .then((response) => {
        const { data } = response;
            ...
            >
              ...
              >
                ...
                <div className="fv-row mb-7">
                  <label className="d-block fw-bold fs-6 mb-1">
                    Discription
                  </label>
