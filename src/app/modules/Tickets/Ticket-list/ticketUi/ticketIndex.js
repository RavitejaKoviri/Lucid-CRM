...
const TicketIndex = () => {
  ...
  const Tickets = TicketsData.filter((val) => {
    ...
    if (
      val?.ticketPriority?.toLowerCase()?.includes(searchTerm?.toLowerCase())
    ) {
      return val;
    }
  });
  ...
  function handleCardMove(board, card, source, destination) {
    axios
      .get(
        `http://65.2.10.157:5377/ticket-statuses?ticketStatusName=${destination?.toColumnId}`,
        {
          headers: {
                      ...
                      <>
                        <div>
                          <button
                            ...
                            className="btn btn-sm btn-icon btn-color-light-dark btn-active-light-primary"
                            onClick={() => {
                              navigation("EditTicket", { state: { id } });
                            }}
                          >
                            {/* begin::Svg Icon | path: icons/duotune/general/gen024.svg */}
                            <svg
                        ...
                        <div>
                          <button
                            ...
                            className="btn btn-sm btn-icon btn-color-light-dark btn-active-light-primary"
                            onClick={() => {
                              dispatch(DeleteTickets(id, token));
                            }}
                          >
                            {/* begin::Svg Icon | path: icons/duotune/general/gen024.svg */}
                            <svg
